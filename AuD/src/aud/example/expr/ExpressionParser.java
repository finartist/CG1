...
/** Simple expression parser.<p>
    ...
    <li><em>The current implementation has a problem that will be
    fixed. Can you find and describe it?</em>
    </li>
  ...
  protected ExpressionTree factor(int level) {
    ...
    if (lookahead()==Tokenizer.NUMBER) {
      tree=new ExpressionTree
                (new Number(Double.parseDouble(scanner_.matchedText())));
    }
    else if (lookahead()==Tokenizer.IDENTIFIER) {
      tree=new ExpressionTree(new Symbol(scanner_.matchedText()));
    }
