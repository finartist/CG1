...
/** Simple implementation of 2-3-4-trees based on {@link KTreeNode}.
   ...
   new node is a 5-node (<em>recursive</em> {@link split_bottom_um}).<br>
   ...
   the parent by {@link KTreeNode#mergeChild}) may differ from the
   correct implementation.</li> </ul>
...
public class A234Tree<Key extends Comparable<Key>> implements Graphvizable {
   ...
   */
  protected void split_bottom_up(KTreeNode<Key> node) {
    if (node.getK()>4) { // CHEATING: we created and split a 5-node!
      ...
      if (node.parent_!=null) {
        ...
        split_bottom_up(node.parent_); // eventually split parent
      }
    }
  }
