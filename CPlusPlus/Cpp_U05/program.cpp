...
&& boolscher vergleich, gibt true zurück, wenn a() UND b() true liefern
& Bitwise Operator, gibt bits zurück, die an beiden Stellen 1 sind  00101101 & 00001100 wird 00001100
...
* ^
* | 
* schaltet Warnings für "unsichere" C Funktionen aus
*
* Nutzt wo moeglich const
* implementiert die angegebenen Operatoren für dynamicObjectList und Object
* Korrigiert den Runtimefehler in dynamicObjectList.sort() -> in Abfrage (i >= 0) immer wahr, weil unsigned int -> ändern zu i < m_count
*
*
*
**/
...
int main()
{
  ...
  for (unsigned int i = 0; i < li1.getCount(); ++i)
  {
	  Object* object = constList.getAt(i);
	  if (object == nullptr)
	  {
		  std::cout << "Object number " << i << " does not exist! This is unexpected!" << std::endl;
	  }
