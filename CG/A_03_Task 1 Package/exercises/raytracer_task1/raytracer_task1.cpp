...
std::shared_ptr<rt::Scene> makeTask1Scene()
{
  ...
  rt::RayIntersection intersectionWithTriangle;
  if(triangle->closestIntersection(ray,maxLambda,intersectionWithTriangle))
  {
    std::cout<< "Ray-Triangle Intersection at "<<std::endl;
    std::cout<<"pos: "<<intersectionWithTriangle.position()<<std::endl;
    std::cout<<"uvw: "<<intersectionWithTriangle.uvw()<<std::endl;
  }
  else
