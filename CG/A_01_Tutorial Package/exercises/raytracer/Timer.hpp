...
public:
  ...
  RAYTRACER_EXPORTS enum { CLOCK = __VC_BASE_CPU_TIME_CLOCK };

  /// create timestamp
  RAYTRACER_EXPORTS cpu_time_t();

  /// resultion() is meaningful only if high resolution timers are used
  RAYTRACER_EXPORTS static bool is_highres() {
    return (CLOCK==__VC_BASE_POSIX_RT_CLOCK) || (CLOCK==__VC_BASE_WIN_HPC);
  }
  /// get resolution in seconds
  RAYTRACER_EXPORTS static double resolution() { return sm_resolution; }

  /// get time
  RAYTRACER_EXPORTS double seconds() const;
   ...
   */
  RAYTRACER_EXPORTS std::string to_s(const char* _fpformat=0) const;
  ...
  RAYTRACER_EXPORTS static void tic();

  /// stop global stop watch (StopWatch::toc())
  RAYTRACER_EXPORTS static cpu_time_diff_t toc();
  /// same, additonally base::oprintf() _msg to _os with %s as to_s()
  RAYTRACER_EXPORTS static cpu_time_diff_t toc(std::ostream& _os,
...
# elif __VC_BASE_CPU_TIME_CLOCK==__VC_BASE_POSIX_CLOCK
  clock_t         m_clock; //!< double time as returned by clock
...
public:
  ...
  RAYTRACER_EXPORTS cpu_time_diff_t(const cpu_time_t& _reference);
};
...
inline cpu_time_t::cpu_time_t() {
  ...
  //int rv=clock_gettime(CLOCK_doubleTIME,&m_tp);
...
RAYTRACER_EXPORTS inline cpu_time_t::cpu_time_t()  {
  int rv=QueryPerformanceCounter(&m_count);
  assert(rv!=0 && "failed to query performance counter");
}

RAYTRACER_EXPORTS inline cpu_time_diff_t::cpu_time_diff_t(const cpu_time_t& _reference) {
  m_count.QuadPart -= _reference.m_count.QuadPart;
}

RAYTRACER_EXPORTS inline double cpu_time_t::seconds() const {
...
public:
  /// start stop watch
  RAYTRACER_EXPORTS void tic() { m_time=cpu_time_t(); }
  /// stop watch
  RAYTRACER_EXPORTS cpu_time_diff_t toc() const {
    return cpu_time_diff_t(m_time);
  }
  /// same, and base::oprintf() _msg to _os with %s as cpu_time_t::to_s()
  RAYTRACER_EXPORTS cpu_time_diff_t toc(std::ostream& _os,
          const char* _msg="elapsed time %s\n");

  /// get global stop watch used by cpu_time_t::tic() (per thread instance)
  RAYTRACER_EXPORTS static StopWatch& global();
...
RAYTRACER_EXPORTS inline StopWatch& StopWatch::global() {
  return __global_StopWatch_global;
}

//-----------------------------------------------------------------------------

RAYTRACER_EXPORTS inline void cpu_time_t::tic() { StopWatch::global().tic(); }
RAYTRACER_EXPORTS inline cpu_time_diff_t cpu_time_t::toc() { return StopWatch::global().toc(); }
RAYTRACER_EXPORTS inline cpu_time_diff_t cpu_time_t::toc(std::ostream& _os,const char* _msg) {
